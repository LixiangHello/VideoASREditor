...
class JianYingASR(BaseASR):
    ...
    def submit(self) -> str:
        ...
        sign, device_time = self._generate_sign_parameters(
            url="/lv/v1/audio_subtitle/submit",
            pf="4",
            appvr="4.0.0",
    ...
    def query(self, query_id: str):
        ...
        sign, device_time = self._generate_sign_parameters(
            url="/lv/v1/audio_subtitle/query",
            pf="4",
            appvr="4.0.0",
            tdid="3943278516897751",
        )
    ...
    def _generate_sign_parameters(
        url: str, pf: str = "4", appvr: str = "4.0.0", tdid: str = "3943278516897751"
    ) -> Tuple[str, str]:
    ...
    def _upload_sign(self):
        ...
        sign, device_time = self._generate_sign_parameters(
            url="/lv/v1/upload_sign", pf="4", appvr="4.0.0", tdid="3943278516897751"
        )
        headers = self._build_headers(device_time, sign)
    ...
    def _upload_auth(self):
        ...
        request_parameters = f"Action=ApplyUploadInner&FileSize={
            file_size}&FileType=object&IsInner=1&SpaceName=lv-mac-recognition&Version=2020-11-19&s=5y0udbjapi"
        ...
        authorization = f"AWS4-HMAC-SHA256 Credential={self.access_key}/{
            datestamp}/cn/vod/aws4_request, SignedHeaders=x-amz-date;x-amz-security-token, Signature={signature}"
    ...
    def _upload_file(self):
        """Upload the file"""
        url = f"https://{self.upload_hosts}/{
            self.store_uri}?partNumber=1&uploadID={self.upload_id}"
    ...
    def _upload_commit(self):
        ...
        url = f"https://{self.upload_hosts}/{self.store_uri}?uploadID={
            self.upload_id}&partNumber=1&x-amz-security-token={self.session_token}"
        headers = self._uplosd_headers()
        response = requests.put(url, data=self.file_binary, headers=headers)
